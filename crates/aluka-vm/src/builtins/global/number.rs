...
pub(crate) fn number_static(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    ...
    match method {
        "isInteger" => { let n = to_num(vm, v); Ok(Value::Boolean(matches!(v, Value::Number(_)) && n.fract() == 0.0 && n.is_finite())) }
        "isSafeInteger" => {
            let n = to_num(vm, v);
            Ok(Value::Boolean(matches!(v, Value::Number(_)) && n.fract() == 0.0 && n.is_finite() && n.abs() <= 9007199254740991.0))
        }
        "isFinite" => { let n = to_num(vm, v); Ok(Value::Boolean(matches!(v, Value::Number(_)) && n.is_finite())) }
