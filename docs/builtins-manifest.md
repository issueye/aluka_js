...
| 39 | `cluster` | 多进程集群 | **[已实现核心]**（真多进程拓扑 + 内核端口共享 + IPC 最小面 + `listen` 错误载体 `Error` 化 + `settings.exec/args/silent/cwd` 生效 + 服务端 `Connection` 语义（该 HTTP 面支撑已于 20260911 收口，见 `http`(29) 行）+ primary 侧生命周期事件 `listening`/`disconnect`/`state`/异步 `fork`（20260911 收口）+ **worker 侧 IPC 面**：`process.on('message')` 接收（2 实参、`handle` 恒 `undefined`）、`process.disconnect()`（`connected` 同步翻转 + `ERR_IPC_DISCONNECTED`）、`process` 真实事件器、`cluster.worker` 事件面与 `require('cluster')` 桥接、**IPC 通道默认保活**（20260911 收口，见 `m52_worker_msg_test.rs`）；真 round-robin 调度未实现，见 §M5） | [`crates/aluka-vm/src/builtins/cluster.rs`](file:///e:/codes/go_projects/aluka_lang/aluka_lang/aluka_r/crates/aluka-vm/src/builtins/cluster.rs) | `conformance_node22_test.rs`（`21-m5-cluster-http` 与 Node 22 逐字节对拍）+ `m52_worker_msg_test.rs`（消息投递 / `process.disconnect` / worker 事件面 / `process` 事件器值语义 / 通道保活，5 例对拍）+ `m52_settings_test.rs`（settings 契约 / exec·args 生效 / 裸 fork 回退 / cwd / silent / validator 文本，7 例对拍）+ `m52_cluster_events_test.rs`（生命周期事件链 + 四形态 listening payload，4 例对拍）+ `m52_http_cluster_test.rs`（EADDRINUSE 全属性对拍 3 例）+ `builtins_phase6_proc_test.rs`（本地锚点，**未与 Node 对拍**） |
...
| 50 | `sqlite` | SQLite 数据库 | **[已完整实现]** | [`crates/aluka-vm/src/builtins/sqlite.rs`](file:///e:/codes/go_projects/aluka_lang/aluka_lang/aluka_r/crates/aluka-vm/src/builtins/sqlite.rs) | `builtins_phase7_io_test.rs`（5 例：4 例本地锚点 + 1 例 `node:sqlite` 能力门控的 Node 真对拍，能力缺失时打印 `[SKIP node-e2e]` 可见跳过，绝不静默假装已对拍） |
...
| 54 | `test` | 原生测试运行器 | **[已实现核心]**（Timer Mock `t.mock.timers` 已实现——`enable`/`tick`/`setTime`/`runAll`/`reset`，与 Node 22 逐字对拍；`apis:['Date']`/`['scheduler.wait']` 未实现；子测试未 `await` 即取消等 Node 细节另有已登记缺口） | [`crates/aluka-vm/src/builtins/test/`](file:///e:/codes/go_projects/aluka_lang/aluka_lang/aluka_r/crates/aluka-vm/src/builtins/test/mod.rs) | `builtins_phase8_compat_test.rs`（6 个 node:test 用例**仅断言本仓自身输出**，未与 Node 对拍）+ `test_runner_cli_test.rs`（值调用形态 / todo / skip / 退码 e2e，6 例）+ `m5_semantics_test.rs`（Timer Mock 3 例与 Node 22 逐字节对拍） |
| 55 | `test/reporters` | 测试报告格式化 | **[已实现核心]**（LCOV 覆盖率输出未实现——`lcov` 仅为最小可写流；报告器流非真 `stream.Transform`，`write` 只转发 `data` 事件） | [`crates/aluka-vm/src/builtins/test_reporters.rs`](file:///e:/codes/go_projects/aluka_lang/aluka_lang/aluka_r/crates/aluka-vm/src/builtins/test_reporters.rs) | `builtins_phase8_compat_test.rs::test_reporters_surface_e2e_matches_go`（**仅断言本仓自身输出**，未与 Node 对拍）+ `test_runner_cli_test.rs`（spec/tap 逐字契约与退码） |
